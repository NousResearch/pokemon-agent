...
# (label, state, axis, hold, rel, off0) combos. off0 is the state's DOMINANT
# trigger offset; reproduce() measures the realized offset and self-corrects, so a
# single per-state offset is enough (no offset-list guessing). Multiple states are
# kept so a candidate can be realized under different intra-frame timings and its
# BEST IV-variant captured.
DEFAULT_COMBOS = [
    ("g3/LR2:1", G3, "LR", 2, 1, 71),
    ("g3/LR1:1", G3, "LR", 1, 1, 98),
    ("ss5/LR2:1", SS5, "LR", 2, 1, 59),
    ("ss5/LR1:1", SS5, "LR", 1, 1, 98),
    ("ss5/LR3:1", SS5, "LR", 3, 1, 100),
    ("env01", "roms/envs/route3_env01.ss1", "LR", 2, 1, 39),
    ("env02", "roms/envs/route3_env02.ss1", "LR", 2, 1, 29),
    ("env03", "roms/envs/route3_env03.ss1", "LR", 2, 1, 19),
]
...
def run_unified_hunt(species, slots, metric, phys_viable, cache, out_state,
    ...
    for G, pid, nat, _bv in rows[:topn]:
        best = None
        for clabel, state, axis, hold, rel, off0 in combos:
            B = make_bundle(state)
            res = reproduce(B, G, pid, species, axis, hold, rel, off0=off0)
            if res:
                iv = tuple(res[1])
                if best is None or metric(iv, nat) > metric(best[0], nat):
                    best = (iv, clabel, state, axis, hold, rel, res[3])
        if best is not None:
    ...
    if realized:
        ...
        B = make_bundle(state)
        # Re-realize the winner (reproduce leaves the core sitting on the encounter)
        # and save that catchable state.
        res = reproduce(B, G, pid, species, axis, hold, rel, off0=off)
        if res and tuple(res[1]) == iv:
